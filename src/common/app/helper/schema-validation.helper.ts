...
export class SchemaValidationHelper {
  ...
  public static async validate(schema: Joi.AnySchema, value: any, convert = false) {
    const { error, value: newValue } = schema.validate(value, { abortEarly: true, allowUnknown: true, convert: convert });
    if (error) {
      this.logger.error(error.message);
      const errors = error.details.map(({ message }) => ({ message: message.replace(/"/g, '') }));
      throw new HttpException({ success: false, message: errors[0].message, statusCode: 400 }, HttpStatus.BAD_REQUEST);
    }
